...
class Order: ObservableObject, Identifiable, Codable, Hashable {
    ...
    // Convenience initializer to convert from SOrder
    convenience init(from sOrder: SOrder) {
        self.init(
            restaurantId: sOrder.restaurantId,
            orderNumber: sOrder.orderNumber,
            id: sOrder.id,
            tableIds: sOrder.tableIds,
            items: sOrder.items,
            status: sOrder.status,
            orderedAt: sOrder.orderedAt,
            numberOfGuests: sOrder.numberOfGuests
        )
        self.paymentStatus = sOrder.paymentStatus
        self.paymentMethod = sOrder.paymentMethod
        self.discountPercentage = sOrder.discountPercentage
        self.taxAmount = sOrder.taxAmount
        self.serviceChargeAmount = sOrder.serviceChargeAmount
        self.amountPaid = sOrder.amountPaid
        self.notes = sOrder.notes
        self.secretCode = sOrder.secretCode
        self.lastUpdatedAt = sOrder.lastUpdatedAt
        self.printedAt = sOrder.printedAt
        self.completedAt = sOrder.completedAt
        self.createdByStaffId = sOrder.createdByStaffId
    }
}
...
struct SOrder: Identifiable, Codable, Hashable {
    @DocumentID var id: String? // Firestore document ID
    var restaurantId: String
    var orderNumber: String // Human-readable order number (e.g., daily sequential)
